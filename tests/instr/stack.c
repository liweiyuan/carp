...
int main()
{
    ...
    carp_value code[] = {
        CARP_I(PUSH),  val0,        CARP_I(PUSH), val1,
        CARP_I(ADD),
        ...
        CARP_I(PUSH),  val0,        CARP_I(INC),

        CARP_I(PUSH),  val0,        CARP_I(DEC),

        CARP_I(LOADR), reg0,        val0,         CARP_I(PUSHR),
        reg0,

        CARP_I(PUSH),  val0,

        CARP_I(PUSH),  val0,        CARP_I(POP),  reg1,

        CARP_I(PUSH),  val0,        CARP_I(PUSH), val1,
        CARP_I(CMP),

        CARP_I(PUSH),  val0,        CARP_I(PUSH), val0,
        CARP_I(CMP),

        CARP_I(PUSH),  1,           CARP_I(PUSH), 2,
        CARP_I(LT),

        CARP_I(PUSH),  2,           CARP_I(PUSH), 1,
        CARP_I(LT),
